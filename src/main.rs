...
use local_mixing::replace::replace::{compress_big_ancillas, compress_external};
use local_mixing::{
    circuit::CircuitSeq,
...
fn main() {
            ...
            Command::new("butterfly")
                ...
                .arg(
                    Arg::new("external")
                        ...
                        .long("external")
                        .action(ArgAction::SetTrue)
                        .help("Use external simplifier tool"),
                ),
        )
        .subcommand(
    ...
    .subcommand(
        Command::new("abbutterfly")
            ...
            .arg(
                Arg::new("sat")
                    .short('s')
                    .long("sat")
                    .help("Use SAT-based local optimization instead of LMDB compression")
                    .action(clap::ArgAction::SetTrue),
            ),
    )
        .subcommand(
        ...
        .subcommand(
            Command::new("dump_db")
                .about("Dump n3m10 database in Simplifier BENCH format")
        )
        .get_matches();
    ...
    match matches.subcommand() {
        ...
        Some(("butterfly", sub)) => {
            let external = sub.get_flag("external");
            let rounds: usize = *sub.get_one("rounds").unwrap();
            let data = fs::read_to_string("initial.txt").expect("Failed to read initial.txt");
            // let seed = OsRng.try_next_u64().unwrap_or_else(|e| {
            //     panic!("Failed to generate random seed: {}", e);
            ...
            if data.trim().is_empty() {
                ...
                main_butterfly(&c1, rounds, &mut conn, 6, external);
            } else {
                let c = CircuitSeq::from_string(&data);
                ...
                .unwrap();
                main_butterfly(&c, rounds, &mut conn, 6, external);
            }
        }
        Some(("bbutterfly", sub)) => {
            let rounds: usize = *sub.get_one("rounds").unwrap();
            ...
            if data.trim().is_empty() {
                ...
                main_butterfly_big(&c1, rounds, &mut conn, n, false, path, &env, false);
            } else {
                let c = CircuitSeq::from_string(&data);
                main_butterfly_big(&c, rounds, &mut conn, n, false, path, &env, false);
            }
        }
        ...
        Some(("abbutterfly", sub)) => {
            ...
            let sat_mode = sub.get_flag("sat");

            if sat_mode {
                println!("╔═══════════════════════════════════════════════════════════════════╗");
                println!("║  SAT-based Local Optimization Mode                                ║");
                println!("╚═══════════════════════════════════════════════════════════════════╝");
            }
            ...
            if data.trim().is_empty() {
                ...
                if bookendless {
                    ...
                    main_butterfly_big(&c1, rounds, &mut conn, n, true, path, &env, sat_mode);
                }
            } else {
                let c = CircuitSeq::from_string(&data);
                if bookendless {
                    ...
                    main_butterfly_big(&c, rounds, &mut conn, n, true, path, &env, sat_mode);
                }
            }
        }
        Some(("reverse", sub)) => {
        ...
        Some(("compress", sub)) => {
            ...
            if sub.get_flag("external") {
                println!("Using external simplifier...");
                acc = compress_external(&acc, n);
            } else {
                let mut stable_count = 0;
                while stable_count < 6 {
                    let before = acc.gates.len();
                    acc = compress_big_ancillas(
                        &acc,
                        1_000,
                        n,
                        &mut conn,
        ...
        Some(("dump_db", _)) => {
            dump_simplifier_db().expect("Failed to dump db");
        }
        _ => unreachable!(),
    }
}
...
fn perm_tables_with_duplicates(
    ...
    for table in tables {
        ...
        let db = match env.open_db(Some(table)) {
            Ok(d) => d,
...
pub fn dump_simplifier_db() -> Result<(), Box<dyn std::error::Error>> {
    let sqlite_path = "circuits.db";
    let n = 3;
    let m = 10;
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok((circuit_blob, perm_blob))
    })?;

    for res in rows {
        let (circuit_blob, perm_blob) = res?;
        let circuit = CircuitSeq::from_blob(&circuit_blob);
        let perm = local_mixing::circuit::Permutation::from_blob(&perm_blob);

        // 1. Compute Truth Tables for 3 wires
        let mut tts = vec![0u32; 3];
        // 3 inputs -> size 8
        for i in 0..8 {
            let out_val = perm.data[i];
            for k in 0..3 {
                // If k-th bit of output is 1
                if ((out_val >> k) & 1) == 1 {
                    // Set i-th bit of tts[k]
                    tts[k] |= 1 << i;
                }
            }
        }

        // 2. SSA Simulation to get wire IDs and Gate strings
        let mut wire_vers = vec![0, 1, 2]; // Initial inputs
        let mut gate_strings = Vec::new();

        // 32-bit integer for gate inputs/outputs is standard in simplifier DB
        // gates start at index 3

        // Wait, simplifier expects gate definition for index 3, 4, ...
        // We track `wire_vers`.
        // If gate outputs to `t`, it creates new version.
        // We convert that new version to the implicitly defined gate index.

        // Simulating:
        // Gate 0 (implied index 3).
        // Inputs: w_ver[t], w_ver[c1], w_ver[c2].
        // Output: w_ver[t] BECOMES 3.

        let mut next_gate_idx = 3;
        for g in &circuit.gates {
            ...
            let op1 = wire_vers[t];
            let op2 = wire_vers[c1];
            let op3 = wire_vers[c2];

            gate_strings.push(format!("ECA57 {} {} {}", op1, op2, op3));

            // Update target wire to point to this new gate
            wire_vers[t] = next_gate_idx;
            next_gate_idx += 1;
        }
        ...
        // output indices (wire versions at ends)
        for k in 0..3 {
            print!(" {}", wire_vers[k]);
        }
        // gates
