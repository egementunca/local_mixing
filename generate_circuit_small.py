#!/usr/bin/env python3
"""Generate a random 64-wire, 100-gate circuit in local_mixing format."""
import random
...
random.seed(123) # New seed
n_wires = 64
n_gates = 100 # Reduced from 1000

gates = [generate_gate(n_wires) for _ in range(n_gates)]
circuit_str = circuit_repr(gates)

# Write to initial_100.txt 
with open('initial_100.txt', 'w') as f:
    f.write(circuit_str)

print(f"Generated {n_gates} gates on {n_wires} wires")
print(f"Saved to initial_100.txt ({len(circuit_str)} chars)")
